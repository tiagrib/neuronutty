...
class SpatialTemporalEncoderLayer(nn.Module):
    def __init__(self, ninp, num_heads, hidden_dim, dropout):
        ...
        self.SpatialMultiheadAttention = MultiheadAttention(ninp, num_heads,dropout, dtype=torch.float32)
        self.TemporalMultiheadAttention = MultiheadAttention(ninp, num_heads, dropout, dtype=torch.float32)
    ...
    def forward(self, x):
        ...
        tx = torch.transpose(x, 0, 1).float() # 120 x 32 x 24 x 128
